...
async function rollDice() {
    ...
    if (
        ...
        alert(
            "All three players must join before rolling."
        );
        return;
    }
    ...
    if (!bothBetsAccepted) {
        alert(
            "The Middleman must accept both bets before rolling."
        );
        return;
    }
    ...
    try {
        ...
        const finalResult =
            generateDice(selectedDiceCount);
        ...
        await updateDoc(
            gameReference,
            {
                ...
                nextRoundAt: Date.now() + 7000, // 7seconds
                history:
                    arrayUnion(newHistoryItem),

                rollNumber:
                    (currentGameData.rollNumber ||
                        0) + 1,
        ...
        // A staff suspension can arrive between the button click and the write.
        // Refresh the room first so the UI becomes disabled instead of showing
        // Firebase's "Missing or insufficient permissions" message.
        try {
            const latestSnapshot = await getDoc(gameReference);
        ...
        alert("The dice could not be rolled.");
