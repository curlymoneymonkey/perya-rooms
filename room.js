...
const ballDropCountChoices = null;
const ballDropCountChoiceButtons = [];
...
const diceCountChoices = null;
const diceCountChoiceButtons = [];
...
const roomGameModeDetails = null;
const roomGameModeCurrentLabel = null;
...
const roomGameBrand = document.getElementById("roomGameBrand");
const roomGameBrandPrimary = document.getElementById("roomGameBrandPrimary");
...
function openCustomerProof(url) {
    ...
    document.body.classList.add("customerProofModalOpen");
}
...
function closeCustomerProof() {
    ...
    document.body.classList.remove("customerProofModalOpen");
}
...
let ballDropFinishedLocally = false;
...
function startPreparingRoll() {
    if (rollStatus) rollStatus.textContent = nextPreparingRollMessage();
...
function startAnimation(amount, { playSound = true } = {}) {
    stopPreparingRoll();
...
function renderCustomerOrders() {
    ...
    visibleOrders.forEach(order => {
        ...
        card.className = "pendingCustomerOrder roomCustomerOrderCard";
        ...
        if (maySeePrivatePayment) {
            ...
            privateDetails.append(
                makeBuyerDetail("💳 Payment", formatCustomerPaymentMethod(order.paymentMethod)),
                makeBuyerDetail("# Ref No.", String(order.paymentReferenceNumber || "Not provided")),
                makeBuyerDetail("🗓 Paid on", formatCustomerPaymentDateTime(order.paymentDate, order.paymentTime))
            );
            card.appendChild(privateDetails);
        }

        if (maySeePrivatePayment && statusValue === "rejected" && order.rejectionReason) {
        ...
        if (maySeePrivatePayment) {
            ...
            totalValue.textContent = `₱${formatCustomerOrderPrice(computedTotal)}`;
            totalAmount.append(totalLabel, totalValue);
        ...
        if (isHost && statusValue === "order_sent") {
...
function revealPermanentViewerRollAfterFallback(marker, amount, finalResult, hadPreparingStage = false) {
    ...
    const beginRealRoll = () => {
        if (viewerFallbackRollMarker !== marker) return;

        rollStatus.textContent = "🎲 Rolling...";
        startAnimation(amount, { playSound: false });
        playRollSound();
    ...
    // If the viewer missed rolling:true, provide a short host-style preparation
    // stage before playing the sound and showing the white shaking dice.
    startPreparingRoll();
    viewerRevealTimer = window.setTimeout(() => {
        viewerRevealTimer = null;
        beginRealRoll();
...
async function resetSharedDiceSession() {
    ...
    previousRenderedHistorySignature = null;

    showWhiteDice(clampDiceCount(currentRoom?.diceCount || 3));
    history.innerHTML = '<p class="emptyHistory">No rolls yet.</p>';
    ...
    try {
        await updateDoc(roomRef, {
            ...
            latestResult: [],
            history: [],
            diceSessionResetAt: serverTimestamp(),
            updatedAt: serverTimestamp()
        });
        ...
        console.error("Could not reset shared Dice session:", error);
    }
}

async function setRoomGameMode(nextMode) {
    if (
        !isHost ||
        !roomRef ||
        currentRoom?.rolling ||
        rollingLocally ||
        currentRoom?.ballDropRolling
    ) return;

    const previousMode =
        currentRoom?.gameMode === "ballDrop" ? "ballDrop" : "dice";
    const gameMode = nextMode === "ballDrop" ? "ballDrop" : "dice";

    // Leaving Dice clears the shared result and Previous Rolls for everyone.
    if (previousMode === "dice" && gameMode !== "dice") {
        await resetSharedDiceSession();
    }
...
window.PeryaRoomGameBridge = {
    getState: () => ({ room: currentRoom, roomRef, isHost, rollingLocally }),
    setGameMode: setRoomGameMode
};

function renderRoomGameBrand(selectedGameMode) {
    const colorBallsSelected = selectedGameMode === "ballDrop";

    if (roomGameBrandPrimary) {
        roomGameBrandPrimary.textContent = "🌐 perya-rooms.com";
    }
if (roomGameBrandSecondary) {
    ...
    if (permanentDiceSkinControl) {
        permanentDiceSkinControl.hidden = colorBallsSelected;
        permanentDiceSkinControl.style.display =
            colorBallsSelected ? "none" : "";
    }

    if (roomGameBrand) {
        roomGameBrand.setAttribute(
            "aria-label",
            colorBallsSelected
                ? "perya-rooms.com, Color Balls"
                : "perya-rooms.com, Dice Skin selector"
        );
    }
}
...
function renderRoom(room) {
    ...
    if (room.ballDropRolling !== true) {
        ballDropFinishedLocally = false;
    }
    const selectedGameMode = room.gameMode === "ballDrop" ? "ballDrop" : "dice";
    ...
    if (rollStatus) {
        rollStatus.hidden = colorBallsSelected;
        rollStatus.style.display = colorBallsSelected ? "none" : "";
        rollStatus.setAttribute(
            "aria-hidden",
            colorBallsSelected ? "true" : "false"
        );

        if (colorBallsSelected) {
            rollStatus.setAttribute("inert", "");
        } else {
            rollStatus.removeAttribute("inert");
        }
    }

    // Room Status belongs only to Color Dice. Completely remove it from
    // layout and accessibility navigation while Color Balls is selected.
    if (roomStatusCard) {
        const hideDiceRoomStatus = selectedGameMode === "ballDrop";

        roomStatusCard.hidden = hideDiceRoomStatus;
        roomStatusCard.style.display = hideDiceRoomStatus ? "none" : "";
        roomStatusCard.setAttribute(
            "aria-hidden",
            hideDiceRoomStatus ? "true" : "false"
        );

        if (hideDiceRoomStatus) {
            roomStatusCard.setAttribute("inert", "");
        } else {
            roomStatusCard.removeAttribute("inert");
        }
    }

    if (hostDiceCountControl) hostDiceCountControl.hidden = !isHost || selectedGameMode !== "dice";
    renderBallDropCountControl(room);
    const gameModeBlocked = Boolean(
        !isHost ||
        room.rolling === true ||
        rollingLocally ||
        (
            room.ballDropRolling === true &&
            ballDropFinishedLocally !== true
        )
    );

    if (roomGameModeSelect) {
        roomGameModeSelect.value = selectedGameMode;
        roomGameModeSelect.disabled = gameModeBlocked;
    }

    if (formalGameIcon) {
        formalGameIcon.textContent =
            selectedGameMode === "ballDrop" ? "⚪" : "🎲";
    }

    if (roomGameModeCurrentLabel) {
        roomGameModeCurrentLabel.textContent =
            selectedGameMode === "ballDrop"
                ? "Color Balls"
                : "Color Dice";
    }

    roomGameModeOptionButtons.forEach(option => {
        const selected =
            option.dataset.gameMode === selectedGameMode;
        ...
        option.disabled = gameModeBlocked;
    });

    if (roomGameModeDetails) {
        roomGameModeDetails.classList.toggle(
            "isDisabled",
            gameModeBlocked
        );

        if (!isHost) {
            roomGameModeDetails.open = false;
        }
    }

    if (roomGameModeHint) {
        roomGameModeHint.textContent = isHost
            ? ""
            : `Current game: ${
                selectedGameMode === "ballDrop"
                    ? "Color Balls"
    ...
    document.body.dataset.roomGameMode = selectedGameMode;
    ...
    rollButton.textContent = room.rollingSuspended === true
        ? "Rolling Suspended"
        : "🎲 Roll Dice";
    const diceControlsDisabled = Boolean(
        !isHost ||
        selectedGameMode !== "dice" ||
        room.rolling ||
        rollingLocally ||
        rollingBlocked
    );
    diceCountSelect.disabled = diceControlsDisabled;
    renderDiceCountChoices(amount, {
        disabled: diceControlsDisabled
    });
    ...
    // Prevent an early snapshot from revealing the host result, but do not
    // impose a fixed reveal timer.
    const revealIsLocked = rollingLocally;

    if (revealIsLocked) {
        // The host is still waiting for the callable response. Keep the current
        // dice visible with the slow preparation wobble; do not let a Firestore
        // snapshot start the real white-dice roll animation early.
    ...
    if (room.rolling) {
        // Viewers mirror the host's slow preparation stage first. The real
        // white-dice animation begins only after Firebase publishes the result.
        if (!isHost) {
    ...
    window.dispatchEvent(new CustomEvent("perya-room-render", {
        detail: { room, isHost, rollingLocally, roomRef }
    }));
    enforceAutoOffline(room).catch(console.error);
}
...
async function chooseRoomGameMode(nextMode) {
    const previous =
    ...
    if (nextMode === previous) {
        if (roomGameModeDetails) roomGameModeDetails.open = false;
        return;
    }

    roomGameModeOptionButtons.forEach(button => {
        button.disabled = true;
    });

    try {
        await setRoomGameMode(nextMode);

        if (roomGameModeSelect) {
            roomGameModeSelect.value = nextMode;
        }
        ...
        if (roomGameModeSelect) {
            roomGameModeSelect.value = previous;
        }

        await showPeryaAlert(
            error?.message || "Could not change the game.",
            {
                type: "error",
                title: "Game Change Failed"
            }
        );
    } finally {
        roomGameModeOptionButtons.forEach(button => {
...
roomGameModeOptionButtons.forEach(button => {
    button.addEventListener("click", () => {
        if (button.disabled || !isHost) return;
        chooseRoomGameMode(button.dataset.gameMode);
    });
});

roomGameModeDetails?.addEventListener("toggle", () => {
    if (
        roomGameModeDetails.open &&
        (!isHost ||
         currentRoom?.rolling ||
         rollingLocally ||
         currentRoom?.ballDropRolling)
...
roomGameModeSelect?.addEventListener("change", event => {
    chooseRoomGameMode(event.target.value);
});

function renderDiceCountChoices(value, { disabled = false } = {}) {
    const amount = clampDiceCount(value);

    if (diceCountSelect) {
        diceCountSelect.value = String(amount);
        diceCountSelect.disabled = disabled;
    }
}
...
diceCountChoices?.addEventListener("click", event => {
    const choice = event.target.closest(".diceCountChoice");
    if (!choice || choice.disabled) return;

    saveDiceCount(choice.dataset.diceCount);
});

diceCountChoices?.addEventListener("keydown", event => {
    const current = event.target.closest(".diceCountChoice");
    if (!current) return;

    const available = diceCountChoiceButtons.filter(button => !button.disabled);
...
ballDropCountDisplay?.addEventListener("change", () => {
    saveBallDropCount(ballDropCountDisplay.value);
});

ballDropCountChoices?.addEventListener("click", event => {
    const choice = event.target.closest(".ballDropCountChoice");
    if (!choice || choice.disabled) return;

    const amount = clampBallDropCount(choice.dataset.ballCount);
    saveBallDropCount(amount);
});

ballDropCountChoices?.addEventListener("keydown", event => {
    const current = event.target.closest(".ballDropCountChoice");
    if (!current) return;

    const available = ballDropCountChoiceButtons.filter(button => !button.disabled);
    const currentIndex = available.indexOf(current);
    if (currentIndex < 0) return;
...
function resetDiceWhenHostExits() {
    if (!isHost || !roomRef) return;

    // Best-effort Firestore write during exit. start() performs the same
    // cleanup on the next host entry in case the browser interrupts this.
    updateDoc(roomRef, {
        rolling: false,
        pendingResult: [],
        latestResult: [],
...
window.addEventListener("pagehide", resetDiceWhenHostExits);

window.addEventListener("beforeunload", () => {
    resetDiceWhenHostExits();
...
async function start() {
    ...
    // Every host room visit starts with a clean Dice session.
    // This also clears stale viewer results if a prior unload write failed.
    if (isHost) {
        await resetSharedDiceSession();
        currentRoom = {
            ...currentRoom,
...
function clampBallDropCount(value) {
    ...
    return Math.min(6, Math.max(1, Math.round(amount)));
}

function renderBallDropCountChoices(value, { disabled = false } = {}) {
    const amount = clampBallDropCount(value);

    if (ballDropCountDisplay) {
        ballDropCountDisplay.value = String(amount);
        ballDropCountDisplay.disabled = disabled;
    }
}

async function saveBallDropCount(value) {
    if (!isHost || !roomRef || currentRoom?.rolling || currentRoom?.ballDropRolling) return;
    const amount = clampBallDropCount(value);
    renderBallDropCountChoices(amount, { disabled: true });
    try {
        await setSecureBallDropCount({
            roomType: "permanent",
            roomId: roomRef.id,
            amount
        });
        ...
        renderBallDropCountChoices(
            currentRoom?.ballDropBallCount || 3,
            { disabled: false }
        );
        showPeryaAlert?.(error?.message || "Could not change the Color Balls count.", {
            type: "error",
            title: "Color Balls"
        });
    }
...
function renderBallDropCountControl(room) {
    const mode = room?.gameMode === "ballDrop" ? "ballDrop" : "dice";
    const amount = clampBallDropCount(room?.ballDropBallCount || 3);
    const blocked = Boolean(
        room?.rolling ||
        rollingLocally ||
        (
            room?.ballDropRolling &&
            ballDropFinishedLocally !== true
        )
    );
    const hidden = !isHost || mode !== "ballDrop";

    if (hostBallDropCountControl) {
        hostBallDropCountControl.hidden = hidden;
    }

    renderBallDropCountChoices(amount, {
